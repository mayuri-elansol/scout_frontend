...
const RoiSelectionModal: React.FC<RoiSelectionModalProps> = ({
  ...
  useEffect(() => {
    ...
    requestAnimationFrame(drawCanvas);
  }, [open, imageLoaded, existingROI, drawCanvas]);
  ...
  //   img.src = cameraFeedUrl || '/img/siteimage.jpg';
  ...
  useEffect(() => {
    ...
    // ✅ prevent reload if same image already loaded
    if (lastLoadedRef.current === cameraFeedUrl) {
      return;
    }

    lastLoadedRef.current = cameraFeedUrl;
    setImageLoaded(false);

    const canvas = canvasRef.current;
    if (!canvas) return;

    console.log('📸 Loading image from:', cameraFeedUrl);
  ...
  useEffect(() => {
    ...
    if (existingROI?.length) {
      const denorm = denormalizeROI(existingROI, canvas);
      setRoiShapes(denorm);
      setHistory([denorm]);
    } else {
      setRoiShapes([]);
  ...
  //   if (!open) return;
  //   drawCanvas();
  // }, [roiShapes, selectedROIIndex, selectedColor, open, drawCanvas]);
