...
const StyledLinearProgress = styled(LinearProgress)<{
...
}>(({ theme, progressvariant, progresssize, progresscolor }) => {
  const heights = {
    small: '6px',
    medium: '8px',
    large: '12px',
  ...
  const progressColor = progresscolor ? colors[progresscolor as keyof typeof colors] : getColorByValue(50);
      ...
      {(showLabel || label) && (
        ...
        >
          {(label || showLabel) && (
            ...
            >
              {label || (variant === 'status' ? getStatusText(percentage) : 'Progress')}
            </Typography>
          )}
          {showPercentage && (
            <Typography
              ...
              sx={{
                fontSize: size === 'small' ? '12px' : '14px',
                fontWeight: 600,
                color: finalColor === 'success' ? '#4caf50' :
                       finalColor === 'warning' ? '#ff9800' :
                       finalColor === 'error' ? '#f44336' :
                       finalColor === 'info' ? '#2196f3' : '#1976d2',
              }}
            >
