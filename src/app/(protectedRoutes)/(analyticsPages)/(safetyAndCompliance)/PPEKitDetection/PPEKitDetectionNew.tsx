...
// import { KpiItem } from "./PPEKitDetection.types";
...
/* ================= TYPES ================= */
type KpiColour = "red" | "blue" | "green";

interface ZoneViolationInteface {
  zone: string;
  violations: number;
  subViolations: { label: string; value: number }[];
}
...
// 📷 Put your downloaded PPE images in public/img/ppe/ with these names
// For card 1 and 3 we use empty imageUrl to show the placeholder gradient.
const MOCK_RECENT_VIOLATIONS: PPEViolation[] = [
  mockViolation(
    "Hard hat missing, Safety glasses missing",
    "Zone A",
    ...
    "14:28:46 · 2 min ago",
    true,
    "", // was "/img/ppe/ppe-1.jpg" – now placeholder
  ),
  mockViolation(
    "Safety vest not worn, Safety glasses missing",
    "Zone B",
  ...
  mockViolation(
    ...
    "14:20:18 · 10 min ago",
    true,
    "", // was "/img/ppe/ppe-4.webp" – now placeholder
  ),
  mockViolation(
    "Hard hat missing, Safety glasses missing",
    "Zone B",
...
const PPEDetectionNew: React.FC = () => {
  ...
  const breakdownTiles = useMemo<BreakdownTile[]>(
    () =>
      MOCK_KPI.map((item) => {
const config = ppeKpiConfig[item.title as keyof typeof ppeKpiConfig];        return {
          label: t(item.title),
          value: item.value,
          icon: config?.icon || EngineeringIcon,
          colour: (item.colour ?? "red") as KpiColour, // ✅ type cast to fix error
        };
      }),
    [t],
  );
