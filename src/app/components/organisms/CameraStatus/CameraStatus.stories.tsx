...
export const Default: Story = {
  parameters: {
    docs: {
      description: {
        story: 'Default camera status as it appears in the SCOUT dashboard showing all zones with their operational status.',
      },
    },
  },
};
...
export const DashboardLayout: Story = {
      ...
      <Box sx={{ 
        ...
        <Box sx={{ 
          ...
          <Box sx={{ textAlign: 'center', color: '#5c6b7d' }}>
            <strong>Activity Feed Component</strong><br/>
  ...
  parameters: {
    docs: {
      description: {
        story: 'Camera Status in dashboard layout showing the actual proportions used in the SCOUT application (35% width).',
      },
    },
  },
};
...
export const CriticalStatus: Story = {
    ...
    const CameraStatusCritical = () => {
      return (
        <Box sx={{ 
          ...
          <Box>
            {[
              { zone: 'Production Floor', active: 6, total: 10, status: 'warning' },
              { zone: 'Chemical Plant', active: 2, total: 8, status: 'warning' },
              { zone: 'Main Entrance', active: 1, total: 3, status: 'warning' },
              { zone: 'Emergency Exits', active: 3, total: 6, status: 'warning' },
  ...
  parameters: {
    docs: {
      description: {
        story: 'Camera status during critical period with multiple zones showing warning status due to camera outages.',
      },
    },
  },
};
...
export const AllOperational: Story = {
    ...
    const CameraStatusOptimal = () => {
      return (
        <Box sx={{ 
          ...
          <Box>
            {[
              { zone: 'Production Floor', active: 10, total: 10, status: 'excellent' },
              { zone: 'Warehouse', active: 6, total: 6, status: 'excellent' },
              { zone: 'Parking Area', active: 5, total: 5, status: 'excellent' },
              { zone: 'Main Entrance', active: 3, total: 3, status: 'excellent' },
              { zone: 'Assembly Line', active: 4, total: 4, status: 'excellent' },
            ].map((zone, index) => (
              <Box
                key={index}
                sx={{
  ...
  parameters: {
    docs: {
      description: {
        story: 'Camera status when all zones are fully operational with excellent status.',
      },
    },
  },
};
