...
//     // await onSubmit?.(combinedFilters);

//     // // 🧹 Run full reset logic
//     // handleReset();
//   };
//   const handleReset = () => {
...
//             const value = opt ?? "";
//             return (
//               <MenuItem key={uuidv4() + index} value={value.toString()}>
//                 {value.toString()}
//               </MenuItem>
//             );
//           })}
...
//             {/* {renderCellValue(
//               column,
//               row[column.id] as string | number | boolean
//             )} */}
//           </TableCell>
//         ))}
    ...
    <Box sx={{ mt: 4, mb: 4 }}>
      <Card sx={{ borderRadius: 2, overflow: "hidden" }}>
        <Box sx={{ p: 3, borderBottom: "1px solid #e0e0e0" }}>
          ...
          <Box sx={{ display: "flex", flexWrap: "wrap", gap: 2, mt: 2 }}>
            {filters.map((filter, index) => (
              <Box key={index} sx={{ flex: "1 1 150px" }}>
                {renderFilter(filter)}
              </Box>
            ))}
            <Box
